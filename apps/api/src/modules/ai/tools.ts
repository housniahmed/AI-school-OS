...
registerTool({
  ...
  execute: async (input: any, ctx) => prisma.asset.findMany({ where: { tenantId: ctx.tenantId, ...(input.category ? { category: input.category } : {}), ...(input.status ? { status: input.status } : {}) }, select: { id: true, assetCode: true, name: true, category: true, status: true, location: true, serialNumber: true }, orderBy: { name: 'asc' }, take: input.limit })
});
...
registerTool({
  ...
  execute: async (input: any, ctx) => { const items = await prisma.inventoryItem.findMany({ where: { tenantId: ctx.tenantId }, select: { id: true, sku: true, name: true, quantity: true, minimumQty: true, unit: true, category: true }, orderBy: { name: 'asc' } }); return items.filter((item) => Number(item.quantity) <= Number(item.minimumQty)).slice(0, input.limit); }
});
...
registerTool({
  ...
  execute: async (input: any, ctx) => {
    ...
    const scored = chunks.map((chunk) => { const text = chunk.content.toLowerCase(); const lexical = terms.reduce((score, term) => score + (text.includes(term) ? 1 : 0), 0); return { chunk, score: lexical / Math.max(terms.length, 1) }; }).filter((x) => x.score > 0).sort((a,b) => b.score - a.score).slice(0, input.limit);
